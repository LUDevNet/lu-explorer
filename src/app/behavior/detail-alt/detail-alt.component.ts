...
@Component({
  ...
  templateUrl: './detail-alt.component.html',
  styleUrls: ['./detail-alt.component.css', "../../../../node_modules/vis/dist/vis-network.min.css"]
})
export class BehaviorDetailAltComponent implements OnInit {
  ...
  {
    this.pending += 1;
    this.luJsonService.getBehavior(id).subscribe(behavior => this.callback(id, level, behavior));
  }
  ...
  add_child(id: number, behavior: Behavior, level: number, key: string, name: string=key)
  {
    if (key && behavior.parameters[key]) this.add_edge(id, behavior.parameters[key], level, name);
  }

  children(id: number, behavior: Behavior, level: number): void
  {
